...
class PredictiveMaintenanceLSTM:
    ...
    def __init__(self, sequence_length=15, prediction_horizon=5):
        ...
        self.failure_patterns = {
            ...
            'Fan_Power': {
                'critical_low': 10.0, 'critical_high': 18.0,
                'warning_low': 11.0, 'warning_high': 17.0,
                'optimal_low': 12.0, 'optimal_high': 15.0,
    ...
    def generate_enhanced_dataset(self, num_samples=5000):
        ...
        for i in range(num_samples):
            ...
            if i < num_samples * 0.6:  # 60% healthy
                ...
                power_base = np.random.normal(13.5, 0.5)
                ...
                power_base = np.random.choice([
                    np.random.normal(11.5, 0.3), # Too low
                    np.random.normal(16.5, 0.3)  # Too high
                ])
                ...
                power_base = np.random.choice([
                    np.random.normal(9.5, 0.2),  # Critically low
                    np.random.normal(18.5, 0.2)  # Critically high
                ])
